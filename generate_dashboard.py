...
def render_html(info, page_info):
    ...
    # [v1.11.2] 导出下拉：主页/历史页给「今天 / 近一周 / 近一月」三范围；
    # 报告页直接导出当前这份周报/月报（"导出范围：今天"放在周报页是错位的，故只留格式列表）。
    # 两处共用同一份 HTML，避免结构漂移。
    if is_report:
        _r_label = "本周报" if report_range == "week" else "本月报"
        export_scope_block = f'      <div class="dropdown-section scope-head">导出{_r_label}</div>\n'
    else:
        export_scope_block = '''      <div class="dropdown-section scope-head">导出范围</div>
...
'''
    export_dropdown_html = f'''<div class="dropdown" id="exportDropdown">
...
{export_scope_block}      <a class="dropdown-item" href="#" data-export="png">PNG 图片</a>
...
{fonts_css_pp}
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  /* v1.11.1：点击展开态（触屏靠 JS 切 .open；桌面 hover 变形不受影响） */
  .report-morph.open .report-face {{ opacity: 0; transform: translateY(-12px); pointer-events: none; }}
  .report-morph.open .report-choices {{ opacity: 1; transform: translateY(0); pointer-events: auto; }}
  .report-choice {{
    color: var(--mist-body);
    font-size: 10px;
  ...
  /* _fix_nav_03：点击 nav 胶囊后，目标 section 着陆瞬间的脉冲高亮，强化“你到了这一组”的锚定感 */
  .reveal-sec.flash::before {{
    content: "";
    position: absolute;
    left: -10px; right: -10px; top: -10px; bottom: -10px;
    border-radius: 16px;
    pointer-events: none;
    box-shadow: 0 0 0 2px var(--accent, #4F5CC7), 0 0 28px 4px rgba(79,92,199,.45);
    opacity: 0;
    animation: secFlash .7s ease-out forwards;
  }}
  .reveal-sec {{ position: relative; }}
  @keyframes secFlash {{
    0%   {{ opacity: 0; }}
    18%  {{ opacity: 1; }}
    100% {{ opacity: 0; }}
  ...
  @media (prefers-reduced-motion: reduce) {{
    .reveal-sec.flash::before {{ animation: none; opacity: 0; }}
  }}
  ...
  /* v1.11.1：删除"下滑隐藏 nav、上滑再出现"整套逻辑。
   * 移动端改为与 web 端完全一致：nav 始终随页面整体上移 → 到顶吸顶 → .is-stuck 渐显下沿金线。
   * 旧 .hide 的 translateY(-160%) 会让 nav 在首次下滑时立刻上跳并消失（用户观感"刚滑就跳一下，
   * 还遮了一下引用块"），是移动端最违和的一处交互。JS 侧切换逻辑已同步删除。 */
  .nav-inner {{
  ...
  /* v1.11.2：分类胶囊单独包一层 .nav-chips。
   * 为什么不能直接让 .nav-inner 横滑：工具组（报告/历史日报/导出）也是 .nav-inner 的
   * 子元素，一旦父级变成滚动容器，工具组会被一起卷进滑动区，滑到右侧才能看到。
   * 包一层之后：胶囊行自己滑，工具组在滑动区之外、独占一行居中。
   * 桌面端仍走 wrap 排布（宽屏放得下，不强行给鼠标用户制造一条要 Shift+滚轮才能拖的滑条）；
   * 移动端见 ≤880px 断点：nowrap + overflow-x: auto，左右滑动选导航。 */
  .nav-chips {{
    display: flex;
    flex-wrap: wrap;
    align-items: center;
  ...
  /* [v1.11.1 删除] 原 @media (max-width: 767.98px) 的 .nav-actions 规则（为旧"chips 横滑、
   * 工具组贴在滑条右侧"的设计服务）：它给工具组铺了一块 var(--ink-card) 底色，在手机视口下
   * 会留下一条突兀的横向色带，而 iPad（820px > 767.98）不触发 —— 同一套布局两种观感。
   * 工具组现在整行居中，底色统一由 .nav.is-stuck 负责，这条规则整体作废。
   * .nav-total-count 的隐藏已合并进 ≤880px 主断点。
   */
  ...
  /* 触屏设备（无 hover）：新闻卡片默认全部展开（触摸只能点，展开态才有意义） */
  @media (hover: none) {{
    ...
    /* v1.11.1：报告胶囊不再"默认摊开成 周报|月报"（摊开后吃掉整行宽度，把同行其他工具挤出去），
     * 改为与 web 一致的「点一下才展开」，由 .open 类控制。 */
    /* v1.11.1 fix：.report-morph 的两个子元素都是 position:absolute，自身宽度完全靠 min-width 撑；
     * 给 0 会塌成 0 宽（.open 后又装不下「周报 | 月报」）。展开态给足 132px。 */
    .report-morph {{ min-width: 72px; }}
    .report-morph.open {{ min-width: 132px; }}
    .report-morph:not(.open) .report-choices {{ opacity: 0; transform: translateY(12px); pointer-events: none; }}
    .report-morph:not(.open) .report-face {{ opacity: 1; transform: translateY(0); pointer-events: auto; }}
  }}
  ...
  .contact-card input, .contact-card textarea {{
    ...
    /* _fix_message：hover 边框略亮、focus 保持高亮但不用金色——品牌蓝 var(--brand-cn) */
    transition: border-color .2s ease, background .2s ease, box-shadow .2s ease;
    outline: none;
  }}
  .contact-card input:hover, .contact-card textarea:hover {{
    border-color: rgba(95, 110, 230, 0.55);
    background: rgba(10, 14, 26, 0.72);
  }}
  .contact-card input:focus, .contact-card textarea:focus,
  .contact-card input:focus-visible, .contact-card textarea:focus-visible {{
    border-color: var(--brand-cn);
    background: rgba(10, 14, 26, 0.78);
    box-shadow: 0 0 0 3px rgba(95, 110, 230, 0.18);
  }}
  ...
  /* _fix_message v2：留言字数限制 — 1000 字上限 + 右下角计数（90% 黄 / 100% 红 + 发送禁用） */
  .contact-card .field-wrap {{ position: relative; }}
  ...
  .contact-card .char-count.warn {{ color: rgba(214, 180, 80, .95); }}
  .contact-card .char-count.over {{ color: rgba(220, 90, 100, .95); font-weight: 600; }}
  .contact-actions button:disabled {{
    opacity: 0.45;
    cursor: not-allowed;
    transform: none !important;
  }}
  .contact-actions button:disabled:hover {{ transform: none; }}
  .contact-actions {{ display: flex; gap: 10px; justify-content: flex-end; margin-top: 18px; }}
  /* _fix_message：取消/发送按钮高度、圆角、padding 统一为 11px 18px + 8px 圆角 */
  .contact-actions button {{
    padding: 11px 20px; border-radius: 8px; font-family: var(--font-base);
  ...
  /* _fix_message：发送留言——模仿 .card-cta「阅读原文」紫色按钮：基类 var(--btn)，hover 高亮 var(--btn-hi) */
  .contact-actions .send {{
    background: var(--btn); color: var(--text-primary); border: 1px solid transparent;
  }}
  .contact-actions .send:hover {{
    background: var(--btn-hi);
    box-shadow: 0 6px 18px -8px rgba(95, 110, 230, 0.55);
  }}
  /* _fix_message：取消按钮——底色加深、描边更亮、hover 中性红（muted red，不刺眼） */
  .contact-actions .cancel {{
    background: rgba(15, 20, 36, 0.72); color: var(--text-primary);
    border: 1px solid rgba(95, 110, 230, 0.55);
  }}
  .contact-actions .cancel:hover {{
    background: rgba(168, 84, 96, 0.55);    /* muted red */
    border-color: rgba(220, 130, 140, 0.75);
    color: #FFE8EB;
  }}
  .contact-close {{
  ...
  .fab-group {{
    ...
    right: 24px;
    bottom: 92px;
    /* v1.11.1：置顶——原 180 低于 nav(196)/ticker(195)，移动端会被 nav 压住，点不到 */
    z-index: 260;
  ...
  /* v1.11.1：hover 态只在真正有指针的设备生效。
   * 触屏浏览器会在按下后把 :hover 粘住 → 点了"上/下"按钮，高亮一直不散。
   * 收进 @media (hover: hover) 后，触屏根本没有 hover 态，只剩 :active 的即时反馈。 */
  @media (hover: hover) {{
    .fab:hover {{
      color: var(--text-primary);
   ...
   * _fix_message：底色、透明度、border 与右上 .fab 三件套一致（rgba(.32) / border-transparent），
   * icon 加 transform: translateY(-2px) 视觉居中——chat-circle-dots 的尾巴偏下让重心下移，
   * 不上移的话 icon 看起来"贴底"不居中。 */
  .fab-contact {{
    position: fixed;
    right: 24px;
    bottom: 42px;            /* 上移，与 .fab-group 间距 = 10px */
    z-index: 260;            /* v1.11.1：与 .fab-group 一起置顶（原 180 会被 nav 压住） */
    -webkit-tap-highlight-color: transparent;
    width: 40px;
    height: 40px;
    border-radius: 50%;
  ...
  /* v1.11.1：同上，触屏收起 hover 展开（避免点完一直保持展开高亮） */
  @media (hover: hover) {{
    .fab-contact:hover {{
      width: 132px;
      border-radius: 999px;
  ...
  /* _fix_message：icon 视觉居中——chat-circle-dots 尾巴偏下，靠 transform 上移 2px 对冲 */
  .fab-contact .fab-icon {{ flex-shrink: 0; transform: translate(5px, -1.5px); }}
  ...
  @media (max-width: 880px) {{
    /* ============ v1.11.1：移动端 nav 与 web 端同构 ============
     * 旧版把 .nav-inner 做成"悬浮玻璃胶囊条"（999px 圆角 + blur + 左右 margin），
     * chips 横向滚动、工具组硬塞同一行 —— 等于移动端自造了一套视觉/交互语言，
     * 且在 iPad 上 5 胶囊 + 共X条 + 3 工具共 9 个元素挤一行必然溢出。
     * 现在：nav 整条默认透明 → 到顶吸顶时整条显底色 + 下沿金线（与 web 完全一致）；
     *       chips 一行（放不下就换行，不再横滑），工具组另起一行居中。
     */
    ...
    /* 跑马灯字号 -2（11px → 9px），左右内边距同步收紧 */
    .ticker {{ margin-top: 0; padding: 6px 0; }}
    .ticker-item {{ font-size: 9px; padding: 0 20px; line-height: 1.5; }}
    .nav {{ padding-top: 0; }}
    /* v1.11.2：nav-inner 改块级堆叠 —— 第一行是可横滑的胶囊行，第二行是工具组。
     * 左右 padding 归零，改由 .nav-chips 自己留内边距，滑动时胶囊能贴到屏幕边。 */
    .nav-inner {{
      background: transparent;
    ...
    /* 分类胶囊：单行横排 + 左右滑动（多胶囊时不换行堆两排，省首屏高度） */
    .nav-chips {{
      display: flex; flex-wrap: nowrap; gap: 8px;
      overflow-x: auto; overflow-y: hidden;
      -webkit-overflow-scrolling: touch;
      overscroll-behavior-x: contain;     /* 滑到两端不把纵滑/浏览器前进后退手势抢走 */
      scrollbar-width: none;
      padding: 2px 20px 4px;              /* 与 .wrap 的 20px 对齐 */
      scroll-padding-left: 20px;
    }}
    .nav-chips::-webkit-scrollbar {{ display: none; }}
    /* 边缘渐隐：只在"那一侧还有没露出来的胶囊"时淡出（JS 打 can-left / can-right），
     * 用来代替滚动条做"还能滑"的提示 */
    .nav-chips.can-left {{
    ...
    .nav-chips.can-left.can-right {{
      -webkit-mask-image: linear-gradient(90deg, transparent 0, #000 24px, #000 calc(100% - 24px), transparent 100%);
      mask-image: linear-gradient(90deg, transparent 0, #000 24px, #000 calc(100% - 24px), transparent 100%);
    }}
    /* nav 字号 -1（chip 13→12 / 序号 12→11 / 计数 12→11） */
    .nav-chip {{ flex-shrink: 0; font-size: 12px; padding: 6px 12px; gap: 8px; }}
    .nav-roman {{ font-size: 11px; }}
    .nav-n {{ font-size: 11px; padding-left: 6px; }}
    .nav-n .count-unit {{ font-size: 10px; }}
    /* "共X条"移动端让位：第二行只放三个工具，居中才好看 */
    .nav-total-count {{ display: none; }}
    /* 工具组：整行独占、居中、字号收小 —— 报告 / 历史日报 / 导出
       （报告页为 周报 | 月报 + 返回今日，同一机制） */
    .nav-actions {{
      position: static; flex-shrink: 0;
      display: flex; justify-content: center;
      /* [v1.11.2] 报告页工具排多一颗「导出」（周报|月报|返回今日|导出），
       * 极窄机型允许换行兜底，不至于撑出视口 */
      flex-wrap: wrap;
      margin: 10px 20px 0; gap: 8px;
    }}
    .report-morph {{ min-width: 72px; height: 28px; }}
    .report-morph.open {{ min-width: 132px; }}
    .report-face {{ font-size: 11px; }}
    .report-choice {{ font-size: 10px; }}
    /* ---- hero：logo 移到右端，与「深南AI日报」同排顶对齐；整排上提 ---- */
    ...
    .wrap {{ padding: 0 20px; }}
    .hero-top {{ padding: 0; margin-bottom: 0; justify-content: flex-end; }}
    .brand-logo .sn-logo {{ width: 38px; height: 38px; object-fit: contain; }}
    /* 负 margin 把「深南AI日报」提到 logo 那一行顶对齐；右侧留出 logo 宽度避免压字 */
    .hero-sub {{ margin-top: -38px; padding-right: 46px; }}
    /* 移动端 hero-grid 改回上下堆叠（去掉 200px 间距） */
    .hero-grid {{ display: block; gap: 0; }}
    .hero-date-col {{ margin-bottom: 16px; }}
    /* v1.11.1 bugfix：报告页 .hero-info-col 基类 margin-top:-60px 在移动端（hero-grid 已改 block）
     * 不再有对齐意义，反而把信息栏上拉 60px 与上方日期/标题重叠 */
    .hero-info-col {{ width: 100%; margin-top: 0; }}
    ...
    /* 底部「聚合报告 / LICENSE / 免责声明…」整排字号 -2（13px → 11px），窄屏允许换行 */
    .footer-docs {{ flex-wrap: wrap; gap: 10px 12px; justify-content: center; }}
    .footer-docs a {{ font-size: 11px; white-space: nowrap; }}
    /* 移动端导出：只保留 PNG（走系统分享面板可"存储到照片"），
     * 文件型格式（HTML/Markdown/CSV/PDF）在手机上取用成本太高，直接隐藏 */
    #exportDropdown .scope-head,
    #exportDropdown .dropdown-divider,
    #exportDropdown [data-export-scope],
    #exportDropdown [data-export]:not([data-export="png"]) {{ display: none; }}
  }}
  @media (max-width: 480px) {{
    ...
    .footer-docs {{ gap: 8px 10px; }}
    .footer-docs a {{ font-size: 10px; }}
...
<div class="contact-modal" id="contactModal" aria-hidden="true">
  <div class="contact-card" role="dialog" aria-modal="true" aria-labelledby="contactTitle">
    ...
    <!-- _fix_message：去掉"天然过滤垃圾信息"和"不会公开你的邮箱"干扰文字，保留核心提交说明 -->
    <p class="tip">留言会以 GitHub Issue 形式提交（需登录 GitHub）。我会在 Issue 里收到通知并回复你。</p>
    ...
    <div class="field-wrap">
      <textarea id="contactMsg" maxlength="1000" placeholder="纠错、合作、建议、想说的……"></textarea>
      <div class="char-count" id="contactMsgCount" aria-live="polite">0/1000</div>
    </div>
...
<script>
  ...
  // _fix_message v2：留言字数限制计数器（textarea 1000 上限 + 0 时发送禁用）
  (function() {{
    const ta = document.getElementById('contactMsg');
    const cnt = document.getElementById('contactMsgCount');
    const send = document.getElementById('contactSend');
    if (!ta || !cnt || !send) return;
    ...
    function update() {{
      ...
      cnt.classList.toggle('warn', !cnt.classList.contains('over') && len >= WARN);
  ...
  (function() {{
    ...
    /* [v1.11.2] 报告页没有"范围"概念：body[data-page-report=week|month] 直接映射到对应载荷。
     * exports.js 里 week = 滚动7天聚合、month = 上个月自然月聚合，与周报/月报页展示内容逐字一致，
     * 因此无需为报告页单独生成一份导出数据。 */
    const __reportRange = document.body.dataset.pageReport;
    let currentScope = __reportRange === 'week' ? 'week' : (__reportRange === 'month' ? 'month' : 'today');
      ...
      el.addEventListener('click', (e) => {{
        ...
        if (fmt === 'html') {{
          ...
          html2canvas(document.body, {{backgroundColor: '#0A0E1A', scale: 2}}).then(canvas => {{
            canvas.toBlob(blob => {{
              const name = `shennan-ai-daily-${{date}}.png`;
              /* v1.11.1：移动端优先调系统分享面板——iOS/Android 的"存储图像"才会真正进相册；
               * 直接 downloadBlob 在手机上只落到"文件"，取用成本高。不支持分享则回退下载。 */
              try {{
                const file = (typeof File !== 'undefined') ? new File([blob], name, {{type: 'image/png'}}) : null;
                if (file && navigator.canShare && navigator.canShare({{files: [file]}})) {{
                  navigator.share({{files: [file], title: '深南AI日报'}})
  ...
  // 2.5) v1.11.1：报告胶囊（"报告" → "周报 | 月报"）点击展开。
  //      旧版触屏（@media hover:none）默认摊开，占掉整行宽度把其他工具挤出去；
  //      现在统一由 .open 类控制：点一下展开 / 点外部收起。桌面 hover 变形照旧保留。
  (function() {{
    const rp = document.getElementById('reportDropdown');
    if (!rp) return;
    rp.addEventListener('click', (e) => {{
      if (e.target.closest('.report-choice')) return;   // 点到 周报/月报 链接：放行跳转
      e.preventDefault();
      e.stopPropagation();
      rp.classList.toggle('open');
    }});
    document.addEventListener('click', (e) => {{
      if (!rp.contains(e.target)) rp.classList.remove('open');
    }});
  }})();
  ...
  // [v1.11.1 删除] 原"移动端滚动时隐藏顶栏/导航（向下滑隐藏、向上滑恢复）"逻辑整体移除。
  //   移动端与 web 端从此共用同一套 sticky 行为：随页面整体上移 → 到顶固定 → 下沿金线。
  ...
  (function(){{
    ...
    // _fix_nav_03：点击胶囊 → 极速位移 + 着陆脉冲，强化分组落点感知（尤其周报/月报）
    // 用 rAF 自写指数缓出，比原生 smooth 更快更可控；落点精确对齐吸顶 nav 高度。
    let scrollAnim = null;
    function easeOutExpo(t) {{ return t === 1 ? 1 : 1 - Math.pow(2, -10 * t); }}
    function goToSection(sec, chip) {{
      if (!sec) return;
    ...
    function landFlash(sec) {{
      sec.classList.remove('flash');
      // 强制 reflow 以便重复点击也能重放动画
      void sec.offsetWidth;
      sec.classList.add('flash');
      setTimeout(() => sec.classList.remove('flash'), 750);
    }}

    // 点击：高亮并锁定，直到下次点选别的胶囊；同时触发极速位移 + 着陆脉冲
    chips.forEach((chip, i) => {{
      chip.addEventListener('click', (e) => {{
        e.preventDefault();                       // 拦截原生锚点跳变，改用特效位移
        manualIdx = i; paint(i);
        revealChip(chip);                         // v1.11.2：被边缘裁掉的胶囊先滚进可视区
        const sec = document.querySelector(chip.getAttribute('href'));
        goToSection(sec, chip);
      }});
    }});
    ...
    // ============================================================
    // 2.5 移动端「胶囊行横滑」（v1.11.2）
    //   需求：分类胶囊多起来时，移动端改为单行横排 —— 左右滑动来选导航，
    //        不再换行堆成两排（两排会白占掉首屏高度）。
    //   这里只做两件事：
    //     ① 点选某颗胶囊后，把它滚进可视区（含被边缘裁掉一半的情况）
    //     ② 某一侧还有没露出来的胶囊时，给那侧加渐隐，提示"还能滑"
    //   注：刻意不跟随页面纵滑自动横移 —— 页面在滚、胶囊行自己在动，晕。
    // ============================================================
    const chipsWrap = document.querySelector('.nav-chips');
    function updateChipsEdge() {{
      if (!chipsWrap) return;
      const max = chipsWrap.scrollWidth - chipsWrap.clientWidth;
      chipsWrap.classList.toggle('can-left', chipsWrap.scrollLeft > 2);
      chipsWrap.classList.toggle('can-right', max > 2 && chipsWrap.scrollLeft < max - 2);
    }}
    function revealChip(chip) {{
      if (!chipsWrap || !chip) return;
      const max = chipsWrap.scrollWidth - chipsWrap.clientWidth;
      if (max <= 2) return;                       // 没溢出，无需横移
      const box = chipsWrap.getBoundingClientRect();
      const pad = 20;                             // 与 CSS 的左右内边距保持一致
      const left = box.left + pad, right = box.right - pad;
      const r = chip.getBoundingClientRect();
      if (r.left < left) {{ chipsWrap.scrollLeft -= (left - r.left); }}
      else if (r.right > right) {{ chipsWrap.scrollLeft += (r.right - right); }}
    }}
    if (chipsWrap) {{
      chipsWrap.addEventListener('scroll', updateChipsEdge, {{ passive: true }});
      window.addEventListener('resize', updateChipsEdge);
      window.addEventListener('load', updateChipsEdge);
      // 字体（ZCOOL XiaoWei 等）晚于首帧加载会把胶囊撑宽，量一次校准
      if (document.fonts && document.fonts.ready) document.fonts.ready.then(updateChipsEdge);
      updateChipsEdge();
    }}
    ...
    // v1.11.1：去掉 rAF 节流 —— scroll 里直接算。
    //   原因：rAF 在"无帧环境"（后台标签页、headless、部分省电模式）下不触发，
    //   is-stuck 就迟迟不上，表现是"吸顶了但金线和底色没出来"。
    //   updateStuck 只读一次 getBoundingClientRect + 两次 classList.toggle，代价极低。
...
__FONTS__
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* v1.11.1：子页面（LICENSE / 免责声明 / 关于项目 / 聚合报告）移动端字号与主页同步收小，
     避免"主页变小、子页没变"的割裂感 */
  @media (max-width: 880px) {
    .doc-wrap { padding: 28px 20px 44px; }
    .doc-home { font-size: 12px; margin-bottom: 20px; }
    .doc-brand { gap: 10px; }
    .doc-brand .sn-logo { width: 38px; height: 38px; }
    .doc-brand h1 { font-size: 24px; }
    .doc-body { margin-top: 20px; }
    .doc-body h2 { font-size: 17px; margin: 22px 0 10px; padding-left: 10px; }
    .doc-body h3 { font-size: 15px; }
    .doc-body p, .doc-body li { font-size: 13px; }
    .doc-pre { font-size: 11px; padding: 12px 14px; }
    .doc-foot { font-size: 10px; margin-top: 34px; }
  }
</style>
</head>
<body>
