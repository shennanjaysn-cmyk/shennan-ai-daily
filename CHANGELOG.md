...
## v1.11.3_260915 — 报告页 hero 归位 + 提示气泡越界归零 + 引用块文案压缩（fix_light_02）

**类型**：修订版本（仅修复 bug / 细节优化）
**日期**：2026-09-15

### 一、报告页 hero 从"错位两栏"收回单栏（修 v1.11.1 引入的排版重叠）
- **症状**：周报/月报页的「周期行 + 引用块」整体与 `.hero-title` 文字带**重叠 48px**，同时引用块左边缘被推离内容列 **200px**，与上方标题左边缘错位。
- **根因**：v1.11.1 去掉金色大字日期后，`.hero-date-col` 变成空节点，但仍占一个 flex 位 ——
  ① `.hero-grid` 的 `gap: 200px` 照样生效，把整个信息栏右推 200px；
  ② 基类 `.hero-info-col { margin-top: -60px }`（设计意图是与大号日期顶对齐）失去参照物，把信息栏**上拉 60px** 压进标题。
- **修法**：报告页新增三条基类覆盖（非媒体查询）——
  `[data-page-report] .hero-date-col { display: none }` + `.hero-grid { display: block; gap: 0 }` + `.hero-info-col { margin-top: 0; width: 100% }`。
  主页两栏由 `[data-page-period]` 独立接管，**不受影响**。
- **结果**：标题底 201px → 周期行顶 213px（12px 间距，零重叠）；周期行/引用块左边缘归位到内容列（28px），右边缘对齐 `.wrap` 右端（与新闻模块齐平）。

### 二、「?」提示气泡改为右缘锚定（修桌面端 30px 横向滚动条）
- v1.11.2 只修了移动端（`@media max-width:880px`），**桌面端仍越界**。
- 气泡 `left:50% + translateX(-50%)` 居中，问号落在行尾时（周报页实测 `L1148`），220px 气泡右缘伸到 `1266 > 视口 1227` → 文档被撑出实测 **30px** 横向滚动条。`opacity:0 / visibility:hidden` **并不豁免**元素参与可滚动溢出区域的计算。
- 改为 `right: -6px` 右缘锚定、向左生长，右边界恒等于问号右缘；移动端那处重复覆盖已删除。

### 三、引用块文案压缩 + 行距收紧
- 行距 `1.85 → 1.72`（长句观感更紧凑）。
- 文案剪掉冗余表述（"自然月，无论今天是几号""全局连续编号""点击"），实测引用块高度 **84px（2 行）→ 56px（1 行）**：
  - 周报：`以下 N 条动态来自最近 7 天（滚动窗口，截止昨天，跨月自然连续）。按版块归类，点卡片直达原文。`
  - 月报：`以下 N 条动态来自上月整月（固定 1 日至月末，跨年自动衔接）。按版块归类，点卡片直达原文。`
  - 主页：`覆盖窗口（北京时间，UTC+8）每日滚动更新。` + `以下 N 条动态按版块归类，点卡片直达原文。`

### 四、验收（headless 几何探针，非肉眼）
| 项 | 结果 |
|---|---|
| 报告页重叠 | 980 / 1100 / 1258 / 1440px **全部归零**（原交叠 48px） |
| 引用块对齐 | 左 28px（= 标题左边缘）、右 1199px（= `.wrap` 右端） |
| 横向溢出 | report-month / report-week / index × 7 档宽度 **全部 0**（原周报 1258px 为 30px） |
| 引用块高度 | 报告页 84px → **56px（1 行）** |
| 移动端（880 / 820 / 500） | 报告页堆叠正常、无重叠、0 溢出 |
| 主页回归 | 两栏结构完好（`.hero-date-col` 465×145 在位）、引用块底 359 = 日期列底（**底对齐保持**） |
| JS 报错 | 0 |
...
- **跑马灯行高兜底**：`.ticker-item` 显式 `line-height: 1.3`（14.3px）。原继承 body 1.6 → 11px 文字行高 17.6px，逼近 32px ticker 容器内容区 16px；OFL 中文字体（如思源黑体/Noto Sans CJK）实际行高偏大时跑马灯文字纵向被裁——"只显示一半"。显式 1.3 后跨字体安全。topbar `top: calc(safe-area + 32px)` 不动
- **移除"送达时间"**：首页 hero 不再展示 `送达时间：{gen_dt}`（每日都变、读者无价值），`hero_meta_block` 在 today 分支置空
- **覆盖窗口去具体时段**：首页 hero lead 改为 `覆盖窗口（北京时间，UTC+8）—— 每日滚动更新`，去掉 `{ws_dt} — {we_dt}`。Markdown 导出（`export_markdown`）同步：删 送达时间行，覆盖窗口改 `每日滚动更新（北京时间，UTC+8）`
- **文档化自动刷新**：页面 JS `setInterval` 每 60s 检测当前时段/日期，接近更新时刻（08:10 / 12:30 / 18:30 北京时间 ±5 分钟）或跨日时 `window.location.reload()` 拉取新数据——读者打开页面后无需手动刷新即享最新。此特性自 v1.10 起即存在，本次正式入文档
